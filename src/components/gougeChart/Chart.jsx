import React, {
  ...
  const ChartComponent = () => {
    const [todayWage, setTodayWage] = useState(350);
    ...
    const guageNeedle = {
      ...
      afterDatasetsDraw(chart, args, plugins) {
        ...
        if (todayWage > yesterdayWage) {
          ...
          if (Math.abs(wageDiff) > halfValue) {
            needleValueAngle = circumference + 1.49 - circumference2(halfValue);
          } else {
            needleValueAngle = circumference + 1.49 - circumference2(wageDiff);
          }
        }
